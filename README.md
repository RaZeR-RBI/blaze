...
[![](coverage.svg)](https://razer-rbi.github.io/blaze/coverage/blaze.c.gcov.html)

[API Documentation](https://razer-rbi.github.io/blaze/api/)
...
* **Dynamic batched drawing**. Designed for moving sprites.
  ...
  /* Flush our batch to screen. The sprites are sorted by their texture */
  BLZ_Present(batch);
  ```
...
* **Render targets**. Allows to draw to textures and use them later, e.g.
 post-processing effects or screen-in-screen rendering.
* **Shaders**. Allows to use custom GLSL shaders and pass parameters to them.
...
# Examples
Check out the **test** directory. It covers all implemented functionality and
uses SDL for window creation.
